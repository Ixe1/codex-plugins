...
When subagent tooling is available, follow `shared/image-gen-workflow.md` to get explicit image-worker authorization before spawning subagents unless the user's request already explicitly authorized subagents. If authorized, run Maquette image creation and image editing in a dedicated image worker subagent, then have the main workflow inspect the returned project-local image path. This applies to brand boards, CSS-contract posters, visual component sheets, page concepts, and generated page raster assets. If image workers are declined, unavailable, or not authorized, continue in the main workflow and record that image-worker handoff was not used.
