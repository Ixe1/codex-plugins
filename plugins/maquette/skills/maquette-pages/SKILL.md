---
name: maquette-pages
description: Create website page concepts from approved brand references using image_gen first, then implement the approved concept with section-by-section visual fidelity review.
---
...
Use this skill after the executable brand canon exists. A broad component library is optional and should not be created for an ordinary first page unless the user explicitly requested Design System Mode, immediate multi-page reuse, or a complex reusable interaction.
...
- If `.maquette/components/component-catalog.json`, `.maquette/components/css/components.css`, `.maquette/components/replica-gallery.html`, or structured component contract evidence exists, consume it only as reusable API context after the page concept has been approved and inventoried. Do not let existing generic component proofs reshape the approved page concept.
- If an existing component catalog lacks reusable component API coverage or marks `assets.reusable_component_review.ready_for_pages` as false, do not block an ordinary page on a component phase. Implement the page-local section faithfully first unless the user requested Design System Mode or immediate multi-page reuse.
- If the component catalog records multiple `assets.sheet_implementation_batches`, verify them only when the current task depends on that reusable catalog. Otherwise ignore incomplete component batches and implement the page from the approved concept and brand canon.
- If the approved page concept needs dense data patterns, complex app-shell behavior, editors, timelines, maps, calendars, or other reusable interactions that are risky to implement page-locally, create or request the missing focused structured contract and browser proof before implementing that region. The contract must be derived from the approved concept region, not from a generic component-gallery idea. Do not generate visual component sheets unless the user explicitly asks for them.
...
For multi-section websites and long pages, a single tall page concept is often too compressed for exact implementation. If any section's typography, card anatomy, media crop, footer/app module, or interaction state is too small or ambiguous to implement faithfully, generate a fresh standalone section/detail concept before coding that section. Do not crop or zoom the old full-page concept as the main reference; regenerate a same-system section reference.
...
6. Before coding, create `.maquette/pages/<page-name>/page-layout-contract.md`.
   ...
   - For strict regions, do not pre-accept generic exceptions such as using a single product lineup crop in place of concept-specific product card scenes. Use concept-matching standalone assets or image edits when the concept shows section-specific product/card imagery, hero imagery, packaging, signage, footer device art, or other branded rasters.
   - If a section/detail concept is needed, record its path and native dimensions in the relevant region before coding.
7. Create `.maquette/pages/<page-name>/asset-manifest.json` before coding.
   ...
   - Reject or retry assets that match the allowed text but visibly fail the concept role, such as product-card crops with blank side bands, hero images with materially different scale/crop from the concept, or footer/app/device art that changes the section anatomy.
   - When a concept already shows a product-card, hero, packaging, poster, sign, or footer/app asset clearly enough, prefer image editing or a concept-matching standalone generation for that exact section role over loose derivative cropping from a broader lineup.
...
8. Decide whether component proof work is justified.
   - Reuse existing components only when they help match the approved concept. Do not copy the `replica-gallery.html` page layout into the page.
   - For ordinary single-page websites, implement repeated sections page-locally with reusable classes inside the page CSS/HTML and record future component candidates after final fidelity review.
   - Run or request `maquette-components` before the page only for explicit Design System Mode, immediate multi-page reuse, or complex reusable interactions such as dense data grids, editors, timelines, maps, calendars, filter builders, or multi-state app shells.
   - If component proof work is justified, the contract must cite the page concept region it serves and preserve that region's fidelity target. It cannot introduce a gallery-derived anatomy, density, or layout that differs from the page concept.
9. Translate the page concept, visual implementation contract, and page layout contract into code using the brand canon and existing helpful components before adding any new composite patterns.
   ...
   - Use the font families, weights, widths, sizes, and line-heights recorded in the design system, brand proof, and concept extraction. If the concept implies condensed or editorial display type, choose a closer available CSS stack or project-approved open-source import instead of defaulting to crude substitutes such as `Impact`.
...
11. Implement and screenshot-review the page in sections when browser tooling is available.
   - Start with the first viewport/header/hero, capture it, compare it directly against the approved concept or section reference, and fix visible drift before continuing when practical.
   - Continue with each major section or tightly related section pair, especially product/repeated-card grids, dense spec/data sections, story/promo bands, newsletter/CTA strips, and the footer.
   - A section pass fails if it uses final empty placeholders, generic ecommerce/SaaS styling, a code-generated identity SVG, uninspected generated assets, loose product crops that visibly differ from the concept, or buttons/type/surfaces that do not match the brand proof.
   - If section-by-section capture is skipped, record the reason in `review.md` and perform an equivalent full-page comparison with explicit top/middle/bottom repair notes before final approval.
12. Update the page blueprint to document composition, concept-region inventory path, visual implementation contract path, page layout contract path, asset manifest path, asset consistency path when present, section-fidelity review status, component strategy, and any new composites.
...
14. Run the required page QA pass:
   ...
   - Fail and fix any strict-region `minor deviation` that is visually repairable. Strict regions may finish as `matches` or `fixed`; otherwise they must be `intentional deviation` with a specific user-approved reason.
   ...
   - A final `minor deviation` status is allowed only for adaptive regions or for non-fixable low-resolution ambiguity. For strict regions, repair it before approval or reclassify it as `intentional deviation` only when the user approved the difference.
...
- In the default first-page workflow, page-local section implementation is preferred over a separate component proof phase. Component extraction happens after the page matches unless Design System Mode, immediate multi-page reuse, or complex reusable interactions justify earlier component contracts.
...
- Generated raster assets must also match their section role. Do not accept a correct-brand product asset when its crop, scale, background, or composition visibly fails the concept's card, hero, story, or footer treatment.
