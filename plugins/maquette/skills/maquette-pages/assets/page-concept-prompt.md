...
- include necessary page-specific composites when the page goal requires them; later implementation will build ordinary sections page-locally first and create reusable component contracts only when Design System Mode, immediate multi-page reuse, or complex interactions justify them
...
- for multi-section pages, make each major section readable enough for direct implementation; if a single overview would make text, button anatomy, product-card crop, footer modules, or icon treatment too small, request coordinated standalone section/detail concepts instead of one compressed page image
...
- make branded raster asset roles clear enough to generate matching standalone assets later; product-card images, hero images, story scenes, newsletter textures, and footer/app/device art should not be interchangeable generic crops
...
The later coded page should match its hierarchy, composition, density, region order, crop intent, product/card anatomy, terminal-section structure, and styling while importing the approved brand canon. Ordinary first-page sections should be implemented directly from this concept; reusable component extraction can be backfilled after the page matches unless the user requested a design system or complex reusable interaction.
