...
The components phase defaults to focused visual component sheets. Maquette generates one 1:1 visual sheet for a component family or pattern, inspects the final downstream artifact, inventories every visible region, derives a reviewable `.maquette/components/contracts/<batch>.contract.css` bridge file deterministically from that visual inspection, implements the full sheet as a componentized batch replica, and uses browser screenshots as the visual correction target. CSS-contract poster images are optional supplemental references, not the primary source of truth.
...
Component implementation includes hard gates per artifact: first make the optional QA tooling decision, then generate one 1:1 visual component sheet by default, inventory every visible region/component/state/pattern, derive a deterministic contract that covers every visible region, render an artifact-specific componentized replica/reference using reusable CSS/JS, write batch artifacts, review the replica against the sheet region-by-region, and only then move to the next artifact.
...
This pass creates a focused core visual component sheet first, inventories every visible region, builds and reviews a faithful componentized visual reference with reusable classes, states, slots, and usage examples, writes batch artifacts, then repeats that loop for focused data/composite/form/navigation/repeated-card/newsletter/footer-social artifacts when the product needs them.
...
Each batch must complete screenshot review or documented manual visual review against its source sheet before Maquette generates the next component artifact. Component sheets are fidelity targets, not loose inspiration: the sheet inventory, deterministic contract, batch replica, catalog, and review must cover every visible region, state variant, responsive variant when shown, and larger pattern. `completed_before_next_sheet: true` is valid only when the replica recreates the full sheet composition, evidence exists, and omissions are zero or explicitly approved and documented.
